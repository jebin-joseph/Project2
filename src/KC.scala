...
object KC extends App {
  ...
  val x = Relation("kc_house_data.csv", "kc_x", null, -1).toMatriD(x_cols)
  val y = Relation("kc_house_data.csv", "kc_Y", null, -1).toVectorD(7)
  val y_mat = Relation("kc_house_data.csv", "kc_Y_Mat", null, -1).toMatriD(y_cols)
  val xy = Relation("kc_house_data.csv", "kc_x_Y", null, -1).toMatriD(all_cols)
