...
object QSAR extends App {
  ...
  val x_cols = Seq(0,1,2,3,4,5,6)
  val y_cols = Seq(7)
  val all_cols = Seq(0,1,2,3,4,5,6,7)
  val x = Relation("qsar_aquatic_toxicity.csv", "qsar_x", null, -1).toMatriD(x_cols)
  val y = Relation("qsar_aquatic_toxicity.csv", "qsar_Y", null, -1).toVectorD(7)
  val y_mat = Relation("qsar_aquatic_toxicity.csv", "qsar_Y_Mat", null, -1).toMatriD(y_cols)
  val xy = Relation("qsar_aquatic_toxicity.csv", "qsar_x_Y", null, -1).toMatriD(all_cols)
  ...
  if (NN_3L) {
    ...
    if(sigmoid_) {
      ...
      new PlotM (t1, rSq.t, Array ("R^2", "R^2 bar", "R^2 cv"), "R^2 vs n for sigmoid tanh nn3l", lines = true)
    ...
    if(tanh_) {
      ...
      val k1 = cols1.size
      val t1 = VectorD.range (1, k1)
      new PlotM (t1, rSq.t, Array ("R^2", "R^2 bar", "R^2 cv"), "R^2 vs n for tanh relu nn3l", lines = true)
  ...
  if (NN_XL) {
    ...
    if(sigmoid_) {
      ...
      val k1 = cols1.size
      val t1 = VectorD.range (1, k1)
      new PlotM (t1, rSq.t, Array ("R^2", "R^2 bar", "R^2 cv"), "R^2 vs n for sig tanh relu nn4l", lines = true)
    ...
    if(tanh_) {
      ...
      val k1 = cols1.size
      val t1 = VectorD.range (1, k1)
      new PlotM (t1, rSq.t, Array ("R^2", "R^2 bar", "R^2 cv"), "R^2 vs n for tanh relu tanh nn4l", lines = true)
      model_perc_0__1.train().eval()
      println(model_perc_0__1.report)
    }
